...
def remove_statistical_outliers(pcd, nb_neighbors=50, std_ratio=2.0):
...
def preprocess_and_merge_pcds(directory, nb_neighbors=50, std_ratio=4.0):
    ...
    merged_pcd = preprocess_pcd(merged_pcd, nb_neighbors, std_ratio)
...
def final_cleanup(filtered_pcd):

    # Remove outliers from the filtered point cloud
    final_cleaned_pcd, _ = filtered_pcd.remove_statistical_outlier(nb_neighbors=50, std_ratio=4.0)

    # Save the cleaned point cloud to the output directory
    output_path = "data/merge_output/final_cleaned_model.pcd"
    o3d.io.write_point_cloud(output_path, final_cleaned_pcd)
    print(f"Final cleaned point cloud saved to {output_path}")

    # Visualize the cleaned point cloud with a coordinate frame
...
def main(directory):
    merged_pcd = preprocess_and_merge_pcds(directory,nb_neighbors=50, std_ratio=4.0)

    if merged_pcd is None:
        print("No merged PCD generated.")
        return

    filtered_pcd, _ = merged_pcd.remove_statistical_outlier(nb_neighbors=50, std_ratio=4.0)

    coordinate_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2, origin=(0, 0, 0))
    o3d.visualization.draw_geometries([filtered_pcd, coordinate_frame], window_name="Final Filtered Point Cloud")

    # Final cleanup step
    final_cleanup(filtered_pcd)
